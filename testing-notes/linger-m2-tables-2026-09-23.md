...
| rule | Room s1 | Chamber s1 | Hall s2 | Cavern s1444 | Plate s3 | Ambience s31 |
|---|---|---|---|---|---|---|
| separation >= 0.9 ms (all 48, 14 sets) | 0.0593 | 0.0166 | 0.1279 | 0.1468 | 0.0069 | 0.0049 |
| core gaps >= 2 % apart | 0.0024 | 0.0004 | 0.0020 | 0.0014 | 0.0005 | 0.0001 |
| no full-band tap 1-8 ms | 300 Hz | 300 | 300 | none in zone | none in zone | 300 |
| Kuttruff 2-20 ms (no dichotic bonus) | 3.34 | 4.34 | 6.95 | 13.12 | 8.58 | 3.23 |
| no tap above -15.3 dB | 1.21 | 1.91 | 6.84 | 11.94 | 8.89 | 0.46 |
| flam (i) after 25 ms, -12 dB | 0.22 | 1.20 | 0.64 | **-1.50 FAIL** | 0.15 | 4.47 |
| flam (ii) no second onset (3 dB) | 2.26 | 2.40 | 1.10 | 0.55 | 0.60 | 1.36 |
| flam (iii) LOC, >= 3 dB below direct | 13.91 | 13.51 | 19.48 | 25.92 | 24.12 | 10.94 |
| flam (iv) inside 5 ms, present and <= 25 % | 0.174 | 0.184 | 0.185 | 0.215 | 0.018 | 0.192 |
| first tap near centre | 0.174 | 0.137 | 0.167 | 0.164 | 0.076 | 0.155 |
| gamma, all seven positions | 0.034 | 0.043 | 0.016 | 0.016 | 0.059 | 0.052 |
| lateral fraction 0.10-0.25 | 0.004 | 0.018 | 0.011 | 0.021 | not held | 0.030 |
...
.288 .131 .105; Cavern .921 .664 .467 .277 .212 .082 .105; Plate .953 .657
.492 .383 .237 .078 .105; Ambience .958 .731 .579 .408 .260 .091 .105. Also
asserted at DENSITY 0 and 100 %, all >= 0, falling >= 0.05 a step. Mono loss
at VARIATION 5 is therefore 10 log10((1 + gamma) / 2) = -2.7 to -2.5 dB.
...
6.6 / 6.5 / 3.5 / 23.2 / 5.8 %. Room against Moorer: 21 core taps (19),
...
at the default ER fader, every tap on, no renormalisation, is 13.9 / 13.5 /
19.5 / 25.9 / 24.1 / 10.9 dB **beyond** the 3 dB it must be below direct
...
73.3 %, Cavern 55.8 %, Plate 61.6 %, Ambience 95.2 % -- every type meets the
old >= 50 % as well. (iv), the small allocation inside 5 ms, has no number in
...
taps, direct at the geometry's own 1 m / d. **Plate's is 0.351, outside both
ranges, and is not asserted**: a plate's images lie in its own plane, so it is
not a room measure there. What the *stereo output* carries -- side over mid,
125-1000 Hz, VARIATION 2 -- is reported beside it: 0.123 / 0.152 / 0.153 /
0.407 / 0.184 / 0.088. Split offsets of 0.24-0.6 ms decorrelate above about a
...
## Plate -- a proposal, flagged

Rewritten after Frosty's research (2026-09-23). A dispersive plate lattice:
2 m x 1 m (EMT 140), driver 0.3 m from the pickup pair, 2D images of orders
0-5, each arrival timed at its band's bending-wave group speed (438 / 196 /
98 / 69 m/s at 10 k / 2 k / 500 / 250 Hz) -- dispersion in feed-forward form,
no allpass. Beta 0.90, gains at an equivalent path 16 m + c t, window 120 ms
at the default SIZE (the table at SIZE 22 m *is* the EMT).

Against the research's checks (VARIATION 2 unless said):

- Instant onset: first tap 0.80 ms left / 0.56 ms right, bright (band 0,
  allowed because it fuses). Heard energy inside 5 ms: 23.2 %.
- Highs first, lows later: band 0 at 0.8 ms; band 1 (2 kHz) mean 22.7 ms
  (9.0-40.5); band 2 mean 62.5 ms (24.9-109.9); dark band mean 77.8 ms
  (12.8-117.7). The order is right; **the span is not**: 0.8-118 ms against
  the research's 1-30 ms.
- **Not met, and why:** the highs are not dense by 5 ms -- 11 section 6
  forbids a full-band tap in 1-8 ms, exactly where a plate is brightest, so
  the first-order edges are darkened and moved to the lows' speed (12-40 ms);
  0.9 ms separation allows at most five taps in 5 ms anyway. The span is long
  because 48 taps at 0.9 ms, placeable at all seven positions, need ~100 ms.
  Per-output low-band L/R offsets are 0.6 ms at most (mean |L - R| 0.48 ms at
  VARIATION 5), not the 5-10 ms the research gives -- the same split ceiling
  as every type.
- Recommendation for M4: Plate's ER is probably best near its -12 dB default
  or lower, with the tail (attack 0, immediate) carrying the onset; its
  default SIZE (22 m placeholder) is now the plate's scale.
...
## Cavern fails flam (i), and it is carried by name

No geometry tried and no seed of 3000 passes (i) at 55 m: the image field
there has the floor bounce, two early walls, then nothing for ~95 ms and a
cluster at 108-135 ms (orders 2-4), which stands above the little energy
before 25 ms. Seed 1444 fails (i) by 1.50 dB and passes everything else. The
test asserts Cavern's (i) margin is no worse than -1.6 dB, by name. At 55 m
orders 1-3 give 26 images in the window and 18 distinct -- fewer than 21 --
so Cavern goes to order 4 (32 images). The research row calls Cavern's early
field "long, sparse, with flutter" -- which is what fails (i). Either the rule
is not meant for Cavern at 55 m, or Cavern's default SIZE comes down; that is
Frosty's call.

## Non-vacuity, observed

- Hand edit of `ErTableData.inc` (Room, VARIATION 3, right, first tap, band
  3 -> 0), rebuild (exit 0), run: **3 assertions red** -- the pin, "Room: no
  full-band tap 1-8 ms", "Room: flam (iv) inside 5 ms". Restored; green.
- Audit weakened by hand in `ErAudit.cpp` (separation threshold 0.9 -> 0,
  Kuttruff -8 dB -> +30 dB), rebuild (exit 0), run: **2 red** -- the two
  hand-broken-table checks for those rules. Restored; green.
- In the suite permanently: one hand-broken copy of Room's table per rule
  (13 rules), each asserted to fail its rule.

## For the engine half and integration

- `windowMs` at 12 m is Room 100, Chamber 66.7, Hall 70.6, Cavern 43.6, Plate
  65.5 and **Ambience 150 -- above its 100 ms clamp**, because every table
